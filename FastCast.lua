-- All of FastCast merged into 1 script because roblox is being a bitch and it keeps getting errors when trying to load the modulescript

local function TypeMarshaller()
...
local function TypeDefinitions()
...
local function Signal()
	local TypeDefs = TypeDefinitions()
...
local ActiveCastStatic = ActiveCast()
local Signal = Signal()
local table = Table()
local ERR_NOT_INSTANCE = "Cannot statically invoke method '%s' - It is an instance method. Call it on an instance of this class created via %s"
local ERR_INVALID_TYPE = "Invalid type for parameter '%s' (Expected %s, got %s)"
